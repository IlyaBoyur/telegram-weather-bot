...
## Планы по улучшению
* **Подключить реальное API** - подключены API Яндекс.Погода и Яндекс.Геокодер
* **Добавить больше городов ( >100 )** - теперь можно задавать любое название города
* Улучшить алгоритм: температура считается комфортной, если она лежит в диапазоне +18 .. +27 градусов Цельсия
* Добавить Telegram бота. Примеры запросов:
1) Какая погода рядом со мной (/my_weather)
2) Какая погода в городе -> Название города (/get_weather)
3) Куда сейчас лучше всего поехать? Топ 1, топ 3, топ 5 городов. (/best_weather)
